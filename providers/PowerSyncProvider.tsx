...
const PowerSyncStatusContext = React.createContext<PowerSyncStatus | null>(
    null,
);
...
export const usePowerSyncStatus = () => {
    ...
    if (!context) {
        throw new Error(
            "usePowerSyncStatus must be used within PowerSyncProvider",
        );
    }
    return context;
};
        ...
        const initializePowerSync = async () => {
            ...
            if (signedIn && !isInitialized) {
                try {
                    ...
                    debugLogger.info(
                        "SYNC",
                        "Starting PowerSync initialization",
                    );
                    await system.init();
                    debugLogger.info(
                        "SYNC",
                ...
                try {
                    debugLogger.info(
                        "SYNC",
                        "Disconnecting PowerSync (user signed out)",
                    );
                    await system.disconnect();
                    setIsInitialized(false);
                    debugLogger.info(
                        "SYNC",
                        "PowerSync disconnected successfully",
                    );
                } catch (err) {
