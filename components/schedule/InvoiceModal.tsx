...
export function InvoiceModal({
  ...
  const markChequeAsPaid = async () => {
    ...
    try {
      ...
      setChequeError(
        error instanceof Error ? error.message : 'Failed to mark cheque as paid. Please try again.'
      );
    } finally {
  ...
  const handlePhoneCall = useCallback((phoneNumber: string) => {
    ...
    Linking.openURL(phoneUrl).catch((error) => {
      ...
      Alert.alert('Error', 'Unable to make phone call. Please try calling manually.');
    });
  }, []);
  ...
  const renderWorkCompletionSection = () => {
    return (
      <View className='flex flex-col gap-6 border-t border-gray-200 dark:border-gray-700 pt-6 mt-6'>
        <Text className='text-xl font-bold text-gray-900 dark:text-white'>Work Documentation</Text>
        ...
        <View className='flex flex-col gap-4'>
          ...
          <View className='flex-row flex-wrap gap-4'>
            ...
            >
              ...
              >
                {hasBeforePhotos ? '✓ Before Photos' : '○ Before Photos Missing'}
              </Text>
            </View>

            <View
              className={`py-2 px-4 rounded-lg ${
                hasAfterPhotos ? 'bg-green-50 dark:bg-green-900/20' : 'bg-gray-100 dark:bg-gray-700'
              }`}
            >
              <Text
                className={`${
            ...
            <View
              className={`py-2 px-4 rounded-lg ${
                hasSignature ? 'bg-green-50 dark:bg-green-900/20' : 'bg-gray-100 dark:bg-gray-700'
              }`}
            >
              <Text
                className={`${
        ...
        <View className='flex flex-col gap-4'>
          ...
          {!hasSignature ? (
            ...
            >
              <Text className='text-white font-medium text-lg'>✍️ Tap to Sign</Text>
            </TouchableOpacity>
            ...
            <View className='p-4 rounded-lg flex-row justify-center items-center bg-blue-600'>
              ...
              <Text className='text-white font-medium text-lg'>📤 Syncing Signature...</Text>
            </View>
          ) : (
            <View className='p-4 rounded-lg flex-row justify-center items-center bg-green-600'>
              <Text className='text-white font-medium text-lg'>✅ Signature Captured</Text>
            </View>
          )}
            ...
            {chequeMarked ? (
              <View className='bg-green-50 dark:bg-green-900/20 p-4 rounded-lg border border-green-200 dark:border-green-800'>
                ...
                {invoice?.paymentDatePaid && (
                  <Text className='text-green-700 dark:text-green-300 text-sm mt-1'>
                    Payment received on: {formatDateReadable(invoice.paymentDatePaid)}
                  </Text>
                )}
                <Text className='text-green-700 dark:text-green-300 text-sm mt-1'>
              ...
              <>
                <TouchableOpacity
                  ...
                  className={`p-4 rounded-lg flex-row justify-center items-center ${
                    isMarkingCheque ? 'bg-gray-400 dark:bg-gray-600' : 'bg-blue-600'
                  }`}
                >
                  {isMarkingCheque ? (
                    <View className='flex-row items-center gap-2'>
                      <ActivityIndicator size='small' color='#ffffff' />
                      <Text className='text-white font-medium text-lg'>Updating...</Text>
                    </View>
                  ) : (
                    <Text className='text-white font-medium text-lg'>💵 Mark Cheque Received</Text>
                  )}
                </TouchableOpacity>
  ...
  const renderPricingSection = () => {
    ...
    return (
      <View className='flex flex-col gap-6 border-t border-gray-200 dark:border-gray-700 pt-6 mt-6'>
        <Text className='text-xl font-bold text-gray-900 dark:text-white'>Invoice Details</Text>

        {/* Invoice items */}
        <View className='flex flex-col gap-4'>
          <Text className='text-lg font-semibold text-gray-900 dark:text-white'>Items</Text>

          {items.length === 0 ? (
            <Text className='text-gray-500 dark:text-gray-400 italic'>No items added</Text>
          ) : (
            <View className='flex flex-col gap-4'>
        ...
        <View className='flex flex-col gap-2 bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg'>
          <View className='flex-row justify-between items-center'>
            ...
            <Text className='text-gray-700 dark:text-gray-300'>${subtotal.toFixed(2)}</Text>
          </View>
          <View className='flex-row justify-between items-center'>
            <Text className='text-gray-500 dark:text-gray-400'>GST (5%)</Text>
            <Text className='text-gray-700 dark:text-gray-300'>${gst.toFixed(2)}</Text>
          </View>
              ...
              <View className='flex flex-col gap-6'>
                ...
                {isManager && (
                  <View className='flex-row justify-between bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg gap-4'>
                    ...
                    <View className='flex flex-col gap-1 flex-1'>
                      <Text className='text-sm text-gray-500 dark:text-gray-400'>Due Date</Text>
                      <Text className='text-sm font-medium text-gray-900 dark:text-white'>
      ...
      <AlertDialog open={showConfirmationModal} onOpenChange={setShowConfirmationModal}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              <Text className='text-muted-foreground'>
                Are you sure you want to send the invoice for "{invoice?.jobTitle || ''}" to the
                client?
              </Text>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction onPress={sendInvoice} disabled={isSendingInvoice}>
      ...
      <AlertDialog open={showChequeConfirmModal} onOpenChange={setShowChequeConfirmModal}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              <Text className='text-muted-foreground'>
                Are you sure you received a cheque payment for "{invoice?.jobTitle || ''}"? This
