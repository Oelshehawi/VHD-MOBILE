...
export function JobPhotoHistory({ scheduleId, jobTitle }: JobPhotoHistoryProps) {
  ...
  const { data: estimatePhotos = [], isLoading: isEstimateLoading } = useQuery<PhotoType>(
    scheduleId
  ...
  const openGallery = useCallback(
    (
      ...
      if (photoType === 'after') {
        ...
        startIndex = jobSection.beforePhotos.length + jobSection.afterPhotos.length;
      }
      ...
      return (
        ...
        >
          <FastImageWrapper
            ...
            showLoader={true}
          />
          <View className={`absolute bottom-0 left-0 right-0 py-1 items-center ${style.bg}`}>
            <Text className='text-white text-xs font-medium'>{style.label}</Text>
          </View>
        </Pressable>
      );
    },
  ...
  const renderPhotoSection = useCallback(
    (photos: PhotoType[], jobSection: JobSection, photoType: 'before' | 'after' | 'signature') => {
    ...
    (jobSection: JobSection) => {
      return (
        <View className='bg-white rounded-xl p-4 mb-4 shadow-sm border border-gray-100'>
          ...
          {renderPhotoSection(jobSection.signaturePhotos, jobSection, 'signature')}
        </View>
      );
    },
    [renderPhotoSection]
