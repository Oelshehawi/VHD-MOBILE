        ...
        <View className='p-4 border-b border-gray-200 bg-gray-50'>
          ...
          <View className='flex-row flex-wrap gap-2 mb-3'>
            ...
            {levels.map((level) => (
              ...
              >
                <Text
                  className={`text-xs ${selectedLevel === level ? 'text-white' : 'text-gray-700'}`}
                >
                  {level.toUpperCase()}
                </Text>
              </TouchableOpacity>
        ...
        <ScrollView className='flex-1'>
          {filteredLogs.length === 0 ? (
            ...
            filteredLogs.map((log, index) => (
              <View key={index} className={`p-3 border-b border-gray-100 ${getLevelBg(log.level)}`}>
                <View className='flex-row justify-between items-start mb-1'>
                  <View className='flex-row items-center flex-1'>
                    ...
                    <Text className='text-xs text-gray-500'>{formatTimestamp(log.timestamp)}</Text>
                  </View>
                </View>

                <Text className='text-sm text-gray-800 mb-1'>{log.message}</Text>
